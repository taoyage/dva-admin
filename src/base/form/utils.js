...
const fieldTypes = {
  ...
  number: ({ meta = {}, inputProps }) => {
    return <InputNumber min={meta.min} max={meta.max} step={meta.step || 1} size={meta.size} {...inputProps} />;
  },
  /**
   * input type="textarea"
  ...
  text: ({ meta = {}, field, placeholder, inputProps }) => {
    placeholder = placeholder === false ? '' : (placeholder || meta.placeholder || `请输入${field.name}`);
    return <Input type="text" placeholder={placeholder} {...inputProps} size={meta.size} />;
  },
  password: ({ meta = {}, field, placeholder, inputProps }) => {
    placeholder = placeholder === false ? '' : (placeholder || meta.placeholder || `请输入${field.name}`);
    return <Input type="password" placeholder={placeholder} {...inputProps} size={meta.size} />;
  }
};
